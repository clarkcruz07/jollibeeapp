...
const RiderMain = () =>{
    ...
    useEffect(() => {
        ...
        const txtLength = document.getElementById('form-input-text').value.length
        if(txtLength > 0){
            setDisable(false)
    ...
    return (
           <div className="container rider">
                <input type="text" className="input-number" id="form-input-text" maxLength='8'  onChange={(e) => setOrderNumber(e.target.value)}/>
                <div className="error-img" id="error">
                    <div>{View}</div>
                    <div><h4>{data}</h4></div>
               </div>
               <div className="network-error hidden" id="network-error">{View}<h4>{errorMsg}</h4></div>
                <Link to="doors" state={{ order_number: orderNumber }}><button className="btn-pin disable" id="btn-done" disabled={disableBtn}><img src={LockBtn}/></button></Link>
                <div className="row header"><h4>How to use Jollibee Smartlockers</h4></div>
