...
const OpenDoor = () =>{
    ...
    console.log(data)
    const openDoor = '/open.svg'
    const closeDoor = '/close.svg'
    const occupiedDoor = '/occupied.svg'
    ...
    return (
        <div className="container">
            <Header />
            <div className="pin-set">{View }<h4> PIN Set: {data}</h4></div> 
            <div className="window">
                    ...
                    .map(
                        item => 
                        {
                            ...
                            if(item.door_status =='1'){
                                const doorStats = <div key={item.id}><img src={closeDoor} onClick={postData(doorNumber, item.door_status, item.order_number, item.id, closeDoor)} />
                                <input type="hidden" value={changeDoorStatus} onChange={(e) => setDoorStatusSet(e.target.value)}/></div>
                                return doorStats
                            }
                            else if(item.door_status =='0'){
                                const doorStats = <div key={item.id}><img src={occupiedDoor} />
                                <input type="hidden" value={changeDoorStatus} onChange={(e) => setDoorStatusSet(e.target.value)}/></div>
                                return doorStats
                            }
                            return doorStats
               ...
               <div><Link to="/staff"> <button className="btn btn-jollibee disable" id="btn-done" disabled={disableBtn}>Done</button></Link></div>
                </div>
            </div>
